...
public class HeartBeatHandler extends ChannelInboundHandlerAdapter {
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        ...
        if (evt instanceof IdleStateEvent) {
            ...
            if (idleStateEvent.state() == IdleState.WRITER_IDLE) {
                ...
                deadFollowers.forEach((k, v) -> ((EzBroker)v).getChannel()
                    .writeAndFlush(MetaData.heartBeat())
                    .addListener((future) -> {
                        if (future.isSuccess()) {
                            recovers.add(k);
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if ((msg instanceof MetaData) && ((MetaData<?>) msg).getType() == MetaDataTypeEnum.HEART_BEAT) {
            ...
            if (beatTime - broker.getLeader().getLastTrans() > BrokerConfig.HEART_BEAT_BEAR) {
                // todo 心跳发晚了，follower以为leader已死，此时可能已经在选举新leader
            }
            broker.getLeader().setLastTrans(beatTime);
            return;
        }
